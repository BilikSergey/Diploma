...
async function resultForReviewForTeacher() {
  const tests_id = db.exec("SELECT * FROM tests WHERE user_id = ?", [
    getUserData().id,
  ]);
  for (let k = 0; k < tests_id[0].values.length; k++) {
    const testId = tests_id[0].values[k][0];
    const submissions_db = db.exec(
      "SELECT * FROM submissions WHERE test_id = ?",
      [testId],
    );
    ...
    if (
      submissions_db &&
      submissions_db[0] &&
      submissions_db[0].values &&
      submissions_db[0].values.length > 0
    ) {
      for (let i = 0; i < submissions_db[0].values.length; i++) {
        ...
        const user_name = db.exec("SELECT username FROM users WHERE id = ?", [
          userId,
        ])[0].values[0][0];
        const test_name = db.exec("SELECT title FROM tests WHERE id = ?", [
          testId,
        ])[0].values[0][0];
        const date = db.exec(
          "SELECT submission_date FROM submissions WHERE id = ?",
          [submissionId],
        )[0].values[0][0];

        let score = 0;
        const student_scores = db.exec(
          "SELECT score FROM option_responses WHERE test_id = ? AND submission_id = ?",
          [testId, submissionId],
        );
...
async function resultForReviewForStudent() {
  ...
  for (let i = 0; i < submissions_db[0].values.length; i++) {
    ...
    const user_id = db.exec("SELECT id FROM users WHERE username = ?", [
      submissions_db[0].values[i][3],
    ])[0].values[0][0];
    const test_name = db.exec(
      "SELECT title FROM tests WHERE id = ? AND user_id = ?",
      [submissions_db[0].values[i][1], user_id],
    )[0].values[0][0];
    const date = db.exec(
      "SELECT submission_date FROM submissions WHERE id = ?",
      [submissions_db[0].values[i][0]],
    )[0].values[0][0];
    let score = 0;
    const questionsDb = db.exec("SELECT * FROM questions WHERE test_id = ?", [
      submissions_db[0].values[i][1],
    ]);
