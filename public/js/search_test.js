...
document.getElementById("searchInput").addEventListener("input", () => {
  ...
  switch (true) {
    case searchInput.length !== 0 &&
      resultsTests.length !== 0 &&
      resultsTeachers.length !== 0 &&
      resultsTeachers[0].values[0][4] === "teacher":
...
function addQueryFromDB(title, author, score, startTime, endTime) {
  ...
  if (timeForStart > now) {
    ...
    btnQuery.onclick = () =>
      alert("The time for passing the exam has not come");
...
function resultOfTestsSearch(resultsTests) {
  for (let i = 0; i < resultsTests[0].values.length; i++) {
    const result = db.exec("SELECT * FROM submissions WHERE test_id = ?", [
      resultsTests[0].values[i][0],
    ]);
    const isTestAlreadyPassedByResultTests =
    ...
    const author = db.exec("SELECT username FROM users WHERE id = ?", [
      resultsTests[0].values[i][1],
    ])[0].values[0][0];
    const startTime = db.exec(
      "SELECT time_of_starting FROM tests WHERE id = ?",
      [resultsTests[0].values[i][0]],
    )[0].values[0][0];
    const endTime = db.exec("SELECT time_of_ending FROM tests WHERE id = ?", [
      resultsTests[0].values[i][0],
    ])[0].values[0][0];
...
function resultOfTeacherSearch(resultsTeachers) {
  const amountOfTestsOfUsers = db.exec(
    "SELECT * FROM tests WHERE user_id = ?",
    [resultsTeachers[0].values[0][0]],
  );
  for (let i = 0; i < amountOfTestsOfUsers[0].values.length; i++) {
    const id_test = db.exec("SELECT id FROM tests WHERE user_id = ?", [
      resultsTeachers[0].values[0][0],
    ])[0].values[i][0];
    console.log(id_test);
    const result = db.exec("SELECT * FROM submissions WHERE test_id = ?", [
    ...
    const isTestAlreadyPassedByResultTeacher =
      result.length > 0 && result[0].values.length > 0
        ? result[0].values[0][0]
        : null;
    if (isTestAlreadyPassedByResultTeacher) continue;
    const title = db.exec("SELECT title FROM tests WHERE user_id = ?", [
      resultsTeachers[0].values[0][0],
    ])[0].values[i][0];
